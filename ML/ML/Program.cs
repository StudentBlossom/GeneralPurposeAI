...
void repeater(object o)
{
    ...
    Console.WriteLine("");
    if (amountofturns % 10 == 0) { Mika.memorytofile(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location).Substring(0, 60) + "GPAImemoryundreadable.bin"); Mika.memorytofile_debugreadable(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location).Substring(0, 60)); }
